                                            ...
                                            uint16_t builtin_routine_id) /**< id of the flag */
{
  ...
  *flags_p = bc_p->header.status_flags;
  ...
  static const uint16_t re_flags[] =
  {
    RE_FLAG_GLOBAL, RE_FLAG_IGNORE_CASE,
    RE_FLAG_MULTILINE, RE_FLAG_STICKY, RE_FLAG_UNICODE
  };
  uint16_t padded_index = (uint16_t) (builtin_routine_id - ECMA_REGEXP_PROTOTYPE_ROUTINE_GET_GLOBAL);
  return ecma_make_boolean_value (*flags_p & re_flags[padded_index]);
                                                ...
                                                ecma_length_t arguments_number) /**< length of arguments' list */
{
...
#if ENABLED (JERRY_BUILTIN_ANNEXB)
  if (builtin_routine_id == ECMA_REGEXP_PROTOTYPE_ROUTINE_COMPILE)
  {
  ...
  switch (builtin_routine_id)
  {
    ...
    /* The routines that do use obj_p */
    ...
    case ECMA_REGEXP_PROTOTYPE_ROUTINE_GET_UNICODE:
    {
      ...
      ecma_value_t ret_value = ecma_builtin_regexp_prototype_flags_helper (re_obj_p, &flags, builtin_routine_id);
      if (ECMA_IS_VALUE_ERROR (ret_value))
      {
        return ret_value;
      }
